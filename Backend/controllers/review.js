...
reviewRouter.post("/", async (req, res) => {
    try {
        let { message, rating, admin_id, user_id } = req.body
        ...
        const { error } = schema.validate({ message, rating, admin_id, user_id });
        if (error) {
            return res.status(400).json({ error: error.details[0].message });
        }
        const reviewExists = await Review.findOne({ admin_id, user_id });
        if (reviewExists) {
            res.status(400).json({ error: 'Already reviewed. Cannot review same admin more than once.' })
        }
        else {
            const review = await Review.create({ message, rating, admin_id, user_id });
...
reviewRouter.get("/", async (req, res) => {
    try {
        ...
        if (admin_id) {
            const review = await Review.find({ admin_id }).populate('user_id');
